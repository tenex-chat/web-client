    ...
    const renderChart = async () => {
      ...
      try {
        ...
        if (err?.message) {
          ...
          if (err.message.includes('Expecting') && err.message.includes('got')) {
        ...
        if (containerRef.current) {
          const escapedChart = chartToRender.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    ...
    <div className={cn(
      ...
      {error && (
        <div className="mb-3 p-3 bg-destructive/10 border border-destructive/20 rounded-md">
          <div className="text-sm font-medium text-destructive mb-1">Failed to render diagram</div>
          <div className="text-xs text-destructive/80 whitespace-pre-wrap font-mono">{error}</div>
        </div>
      )}
 ...
 */
export function detectMermaidDiagram(content: string, language?: string): boolean {
