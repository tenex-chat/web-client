...
export function useLLMConfig() {
  ...
  const setSpeechConfig = useCallback((speechConfig: SpeechConfig, credentials: LLMCredentials) => {
    setConfig(prev => ({
      ...prev,
      speech: {
  ...
  const getConfigForUseCase = useCallback((useCase: string): LLMConfig | null => {
  ...
  const getCredentials = useCallback((provider: LLMProvider): LLMCredentials | null => {
    return config.credentials[provider] || null;
  }, [config]);

  // Get speech configuration
  const getSpeechConfig = useCallback((): { config: SpeechConfig; credentials: LLMCredentials } | null => {
