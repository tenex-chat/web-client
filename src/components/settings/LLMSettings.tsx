...
function LLMConfigForm({ provider, onSave, onCancel, existingConfig }: LLMConfigFormProps) {
    ...
    const [contextWindowSize, setContextWindowSize] = useState(
        existingConfig?.contextWindowSize?.toString() || ""
    );
    ...
    const handleSave = () => {
        ...
        if (contextWindowSize) {
            config.contextWindowSize = Number.parseInt(contextWindowSize);
        }
        if (temperature) {
            config.temperature = Number.parseFloat(temperature);
        }
        if (maxTokens) {
            config.maxTokens = Number.parseInt(maxTokens);
        }
...
export function LLMSettings() {
    ...
    const handleSaveConfig = useCallback(
        (configName: string, llmConfig: LLMConfig) => {
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            <SelectTrigger className="w-full">
                                <SelectValue>{PROVIDER_NAMES[selectedProvider]}</SelectValue>
                            </SelectTrigger>
                            <SelectContent>
            ...
            {!hasConfigurations && !showAddForm && !isLoadingModels && (
                <Card className="p-8">
                    <div className="text-center space-y-2">
                        ...
                        <p className="text-muted-foreground">
                            Add your first LLM configuration to enable AI features like automatic
                            title generation.
                        </p>
