...
describe('projectStatusUtils', () => {
  ...
  describe('bringProjectOnline', () => {
    ...
    it('should throw error if ndk is missing', async () => {
      await expect(bringProjectOnline(mockProject, null as any)).rejects.toThrow('Missing required parameters')
    })

    it('should throw error if project is missing', async () => {
      await expect(bringProjectOnline(null as any, mockNDK)).rejects.toThrow('Missing required parameters')
    })
