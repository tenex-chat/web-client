...
class AIService {
  ...
  async cleanText(text: string): Promise<string> {
    ...
    try {
      ...
      const model = this.currentConfig.model || this.getDefaultModel(this.currentConfig.provider)
...
Text to clean: ${text}`,
  ...
  async transcribe(audio: Blob, openAIKey?: string): Promise<string> {
    const apiKey = openAIKey || 
      (this.currentConfig?.provider === 'openai' ? this.currentConfig.apiKey : null)
  ...
  async speak(text: string, voiceId: string, voiceProvider: 'openai' | 'elevenlabs', apiKey: string): Promise<Blob> {
