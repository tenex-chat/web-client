...
export function LoginForm() {
  ...
  const handleExtensionLogin = async () => {
    if (!hasNip07) {
      toast.error(
        "No Nostr extension detected. Please install Alby, nos2x, or another NIP-07 compatible extension.",
      );
      return;
    }
    ...
    try {
      ...
      toast.error(
        `Failed to login with extension: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <TabsContent value="nsec">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Your private key is stored locally and never sent to any
                  server
                </p>
              </div>
