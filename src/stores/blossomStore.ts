...
export const activeUploadsAtom = atom(
  (get) =>
    get(uploadQueueAtom).filter((item) => item.status === "uploading").length,
);

// Failed uploads atom
export const failedUploadsAtom = atom((get) =>
  get(uploadQueueAtom).filter((item) => item.status === "failed"),
);

// Upload history atom (last 50 successful uploads)
export const uploadHistoryAtom = atom<
  Array<{
    id: string;
    url: string;
    filename: string;
...
export const addToUploadQueueAtom = atom(null, (get, set, file: File) => {
...
export const updateUploadItemAtom = atom(
  ...
  (
    get,
    set,
    { id, updates }: { id: string; updates: Partial<UploadQueueItem> },
  ) => {
    set(
      uploadQueueAtom,
...
// Remove upload item
export const removeUploadItemAtom = atom(null, (get, set, id: string) => {
  set(
    uploadQueueAtom,
...
export const updateServerHealthAtom = atom(
  ...
  (
    get,
    set,
    { url, health }: { url: string; health: Partial<ServerHealth> },
  ) => {
...
// Retry failed upload
export const retryUploadAtom = atom(null, (get, set, id: string) => {
...
// Cancel all uploads
export const cancelAllUploadsAtom = atom(null, (get, set) => {
