...
test.describe("Task creation flow", () => {
	test("should create a new account, project, and task", async ({ page }) => {
		...
		await expect(
			page.getByRole("button").filter({ hasText: "New Task" }),
		).toBeVisible();
		await expect(
			page.getByRole("button").filter({ hasText: "Voice" }),
		).toBeVisible();
		await expect(
			page.getByRole("button").filter({ hasText: "Thread" }),
		).toBeVisible();
		...
		console.log(
			`Successfully created project "${projectName}" and task "${taskTitle}"`,
		);
	});
});
