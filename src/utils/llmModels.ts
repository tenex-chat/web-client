...
export async function getAllModels(): Promise<LLMModelOption[]> {
  const providers: LLMProvider[] = ['openai', 'anthropic', 'google', 'groq', 'deepseek', 'openrouter', 'ollama'];
 ...
 */
export async function testLLMConfiguration(provider: LLMProvider, apiKey: string, model: string): Promise<boolean> {
  try {
