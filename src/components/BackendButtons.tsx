...
export function BackendButtons({ taskId, projectTagId }: BackendButtonsProps) {
    ...
    const [triggeredBackends, setTriggeredBackends] = useState<Set<string>>(new Set());
    ...
    if (onlineBackends.size === 0) {
        return (
            <div className="text-center py-4">
                <p className="text-sm text-muted-foreground">No backends are currently online</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-3">
            <h4 className="text-sm font-medium text-foreground mb-2">Available Backends</h4>
...
function BackendButton({ backendName, onStart, isTriggered }: BackendButtonProps) {
    return (
        <Button
            onClick={onStart}
        ...
        >
            ...
            {isTriggered ? <Check className="w-4 h-4 ml-2" /> : <Play className="w-4 h-4 ml-2" />}
        </Button>
    );
}
