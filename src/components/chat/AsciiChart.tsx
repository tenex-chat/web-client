...
export function AsciiChart({ content, className }: AsciiChartProps) {
  ...
  if (!isLikelyChart) {
    ...
    return <pre className={cn("whitespace-pre-wrap font-mono text-xs", className)}>{content}</pre>;
  }
...
export function detectAsciiChart(content: string, language?: string): boolean {
  ...
  if (language && ['chart', 'diagram', 'ascii', 'flowchart'].includes(language.toLowerCase())) {
    return true;
  }
