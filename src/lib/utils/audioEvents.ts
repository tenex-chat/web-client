...
export function isAudioEvent(event: NDKEvent): boolean {
  // NIP-94 audio events
  if (event.kind === 1063) {
    const mimeType = event.tags.find(
      (tag) => tag[0] === "m" && tag.length > 1,
    )?.[1];
...
export function getAudioDuration(event: NDKEvent): number {
  // NIP-94 events
  if (event.kind === 1063) {
    const durationTag = event.tags.find(
      (tag) => tag[0] === "duration" && tag.length > 1,
    )?.[1];
  ...
  if (event.kind === 1222 || event.kind === 1223) {
    ...
    if (imetaTag) {
      const durationPart = imetaTag.find(
        (part) => typeof part === "string" && part.includes("duration"),
      );
      if (durationPart) {
...
export function getAudioWaveform(event: NDKEvent): number[] {
  // NIP-94 events
  if (event.kind === 1063) {
    const waveformTag = event.tags.find(
      (tag) => tag[0] === "waveform" && tag.length > 1,
    )?.[1];
  ...
  if (event.kind === 1222 || event.kind === 1223) {
    const imetaTag = event.tags.find((tag) => tag[0] === "imeta");
    if (imetaTag) {
      const waveformPart = imetaTag.find(
        (part) => typeof part === "string" && part.includes("waveform"),
      );
      if (waveformPart) {
