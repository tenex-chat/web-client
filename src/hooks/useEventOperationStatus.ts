...
export function useEventOperationStatus(eventId?: string, projectId?: string) {
  ...
  const { isActive, agentCount } = useMemo(() => {
    ...
    // Check TTL (20 seconds as per original store)
    const now = Date.now()
    const eventTime = latestSnapshot.createdAt * 1000
    const isWithinTTL = (now - eventTime) <= 20000

    const hasAgents = latestSnapshot.agentPubkeys.length > 0

    return {
      isActive: isWithinTTL && hasAgents,
