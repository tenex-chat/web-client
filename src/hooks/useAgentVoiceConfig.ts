...
export function useAgentVoiceConfig(agentPubkey: string | undefined) {
  ...
  const [agentConfigs, setAgentConfigs] = useState<Map<string, AgentVoiceConfig>>(() => loadAgentVoiceConfigs())
  ...
  const effectiveConfig = {
    ...
    apiKey: globalVoiceSettings.provider === 'openai' ? openAIApiKey : globalVoiceSettings.apiKey
  }
  ...
  const saveConfig = useCallback((config: Partial<AgentVoiceConfig>) => {
  ...
  // Test the voice
  const testVoice = useCallback(async (text: string = "Hello, this is a test of my voice.") => {
