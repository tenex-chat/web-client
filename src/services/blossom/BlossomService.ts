...
export interface BatchUploadOptions {
  ...
  onAllComplete?: (
    results: Array<{ url: string; metadata: FileMetadata } | Error>,
  ) => void;
  compress?: boolean;
  maxSizeMB?: number;
  maxWidthOrHeight?: number;
...
export class BlossomService {
  ...
  async uploadFile(
    ...
    try {
      ...
      if (
        compress &&
        isImageFile(file) &&
        file.size > this.compressionThreshold
      ) {
      ...
      const server = await this.serverRegistry.selectBestServer(
        processedFile.size,
      );
      if (!server) {
      ...
      const uploadFile = new File([buffer], processedFile.name, {
      ...
      await this.blossomClient.upload(uploadFile, {
        ...
        onProgress: (progress: { loaded: number; total: number }) => {
          ...
          const uploadProgress =
            Math.round((progress.loaded / progress.total) * 90) + 10;
  ...
  ): Promise<Array<{ url: string; metadata: FileMetadata } | Error>> {
    const {
      files,
      onProgress,
      onFileComplete,
