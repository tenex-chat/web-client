...
export function SpeechSettings() {
  ...
  const handleTest = async () => {
    if (!apiKey || !validateApiKeyFormat(provider === 'openrouter' ? 'openrouter' : 'openai', apiKey)) {
      setTestResult(false);
      return;
    }
      ...
      <Card className="p-6">
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Select value={provider} onValueChange={(value: SpeechProvider) => setProvider(value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {Object.entries(SPEECH_PROVIDER_NAMES).map(([providerKey, name]) => (
          ...
          <div className="space-y-2">
            ...
            {!validateApiKeyFormat(provider === 'openrouter' ? 'openrouter' : 'openai', apiKey) && apiKey && (
              <p className="text-sm text-yellow-600">
                API key format may be incorrect for {SPEECH_PROVIDER_NAMES[provider]}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-muted-foreground">
              Specify language code for better accuracy, or leave empty for automatic detection
            </p>
          </div>
          ...
          {currentConfig && (
            <div className="mt-4 p-4 bg-muted/50 rounded-lg">
              ...
              <div className="text-sm space-y-1">
                <p><span className="font-medium">Provider:</span> {SPEECH_PROVIDER_NAMES[currentConfig.config.provider]}</p>
                <p><span className="font-medium">Model:</span> {currentConfig.config.model}</p>
                {currentConfig.config.language && (
                  <p><span className="font-medium">Language:</span> {currentConfig.config.language}</p>
                )}
              </div>
            </div>
          )}
