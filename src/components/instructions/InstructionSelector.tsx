...
export function InstructionSelector({
    ...
    const [previewInstruction, setPreviewInstruction] = useState<NDKLLMRule | null>(null);
    ...
    const handleInstructionSelect = (instruction: NDKLLMRule, agentNames: string[]) => {
    ...
    const handleInstructionDeselect = (instruction: NDKLLMRule) => {
        onInstructionsChange(selectedInstructions.filter((i) => i.id !== instruction.id));
    };
